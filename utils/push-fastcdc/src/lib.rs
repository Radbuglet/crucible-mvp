...
pub struct GearTables {
    value: Option<Box<([u64; 256], [u64; 256])>>,
}
...
impl GearTables {
    ...
    /// perform a bitwise exclusive OR on the values using the given seed. If the
    ...
    pub fn new(seed: u64) -> Self {
        ...
        let mut gear_pair = Box::new((GEAR, GEAR_LS));

        let gear = &mut gear_pair.0;
        let gear_ls = &mut gear_pair.1;
        ...
        Self {
            value: Some(gear_pair),
        }
    }

    pub fn get(&self) -> GearTablesRef<'_> {
        match &self.value {
            Some(gear_pair) => {
                let (gear, gear_ls) = &**gear_pair;

                GearTablesRef { gear, gear_ls }
            }
            None => GearTablesRef::new(),
        }
    }
}

#[derive(Debug, Copy, Clone, Hash, Eq, PartialEq)]
pub struct GearTablesRef<'a> {
    pub gear: &'a [u64; 256],
    pub gear_ls: &'a [u64; 256],
}

impl Default for GearTablesRef<'_> {
    fn default() -> Self {
        Self::new()
    }
}

impl<'a> GearTablesRef<'a> {
...
/// chunk size values given to the [`GearConfig`] constructor in that case.
///
...
impl GearState {
    ...
    pub fn reset(&mut self) -> GearState {
        mem::take(self)
    }
